...
- **没有 `updatedAt`**：日志是 append-only 的不可变记录，唯一会变的是 `is_read`（批量置已读），行级 `updatedAt` 无意义。这是全库第一张没有 `updatedAt` 的表，**有意为之**。
- **已读 / 未读用逐条 `is_read` 布尔**（已确认，否决全局时间游标 `last_read_at`）：可精确标记「我看到的这批」，随后新到的不会被误吞。
- **`source` 尽力而为**：已登录请求可由 auth 中间件区分 web（Cookie）vs cli / mobile（Bearer）；登录事件本身无法区分来源端，留 `unknown`（客户端可带 `X-Client` 头提示，可选）。
...
**401 去重（防扫描器刷屏）**：`auth.unauthorized` 按 IP 限频，同一 IP 在窗口（默认 10 分钟）内只记第一条，窗口外重置。状态在内存 Map，复用 `auth.domain` 的 throttle 清理模式（`_sweep`），防 Map 无限增长。

**刻意不记录**：读操作（列表 / 详情 / `me`）、未变更数据的重复写、MCP 内部直连操作（MCP 已停更）。
...
- **auth 事件在 handler 记录**（需要 `clientIp(c)`）；**删除 / 上传事件在 service 记录**（顺带覆盖 MCP 直连路径，保持一致性）。
- 不记录读操作（见 §4）。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/audit/logs` | 列表（`?page=&pageSize=`，默认 10 最大 50，对齐现有模块；+ `?level=` & `?event=` & `?unreadOnly=`，`created_at DESC`） |
| GET | `/api/audit/logs/unread-count` | `{ unreadCount }`，Web 角标轮询 |
| PUT | `/api/audit/logs/read` | 全部置已读；body 可带 `{ ids }` 精准标记；返回 `{ updatedCount, unreadCount }` |
...
- **默认**：保留 90 天且最多 5000 条，超出部分后台清扫删除（`auditService` 内 `setInterval`，unref；服务启动时也扫一次）。
...
| `audit.domain.ts` | 纯函数：消息构建、401 去重状态机（无 DB / IO） |
| `audit.service.ts` | `auditService`：`record` / `list` / `unreadCount` / `markRead` / `sweep` |
...
然后 `bun run db:generate` / `db:migrate`，`bun run typecheck` 与 `bun test`。
...
| ④ | 记录范围 | `auth.login` / `auth.login_failed` / `auth.logout` / `auth.unauthorized` / `blob.upload` / `blob.delete` / `diary.delete` / `moment.delete` / `task.delete` / `event.delete` |
...
| ⑩ | 事件写入位置 | auth 在 handler（需 IP）；删除 / 上传在 service |
