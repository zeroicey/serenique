---
name: deploy-agent
description: Serenique deployment and CI/CD expert. Use when the requirement involves Docker builds/images, docker compose, GitHub Actions workflows, Docker Hub releases, version tags, or server deployment.
mode: subagent
---
...
- Docker / docker compose (API + MCP images)
...
- Images run as non-root (UID 10001); `BLOB_ROOT=/data/blobs` persisted via the `blob-data` volume
- Runtime env loaded from the project-root `.env` (docker compose), no service-local `.env`
- `scripts/docker-entrypoint.sh` rewrites the localhost DB host to `host.docker.internal`

## Responsibilities

- Dockerfile / docker compose maintenance
...
- `docker compose up -d` (without `--build`) needs no proxy args; the Dockerfile stays registry-agnostic and builds on any network
...
2. After changes, at minimum validate: `docker compose config`, a local `docker build`, and the workflow YAML syntax
