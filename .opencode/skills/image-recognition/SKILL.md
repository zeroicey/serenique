...
```sh
node .opencode/skills/image-recognition/vision.js "<图片路径>" "<问题>"
# 或图片 URL：
node .opencode/skills/image-recognition/vision.js --url "<图片链接>" "<问题>"
```

- 不传问题时默认："请详细描述这张图片的内容。"
- 支持本地路径（jpg/png/gif/webp/bmp）或 `--url` 图片链接
- 配置在同目录 `.env`（DASHSCOPE_API_KEY / VISION_MODEL / DASHSCOPE_BASE_URL），该文件已 gitignore，不要提交
