...
/// 闪记标签（对齐 services/api 的 TagEntry）：name 已按后端规范 trim + 小写归一化，
/// 显示即存储名。momentCount = 绑定该标签的闪记数（当前唯一 ownerType）。
class MomentTag {
  const MomentTag({
    required this.id,
    required this.name,
  ...
  factory MomentTag.fromJson(Map<String, dynamic> json) => MomentTag(
...
class Moment {
  ...
  /// 内嵌标签（对齐 services/api MomentEntry.tags）。列表/详情接口均返回。
  final List<MomentTag> tags;
