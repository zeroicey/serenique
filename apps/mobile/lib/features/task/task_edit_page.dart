// 任务新建/编辑合一全屏页（替代原底部弹窗）。
// [task] 非空 = 编辑；[groupId] 预设所属组（组详情 FAB 新建）；
// [presetDueDate] 预设截止日期（日期 tab 新建时预填）。由 /task/edit 路由承载（ShellRoute 外）。
...
class TaskEditArgs {
  const TaskEditArgs({this.task, this.groupId, this.presetDueDate});

  final TaskEntry? task; // 编辑时回填
  final String? groupId; // 新建时预设所属组
  final String? presetDueDate; // 新建时预设截止日期
}

class TaskEditPage extends ConsumerStatefulWidget {
  const TaskEditPage({super.key, required this.args});
...
class _TaskEditPageState extends ConsumerState<TaskEditPage> {
  late final TextEditingController _title =
      TextEditingController(text: widget.args.task?.title ?? '');
  String? _groupId;
  String? _dueDate;
  String _status = 'todo';
  ...
  void initState() {
    super.initState();
    _groupId = widget.args.task?.groupId ?? widget.args.groupId;
    _dueDate = widget.args.task?.dueDate ?? widget.args.presetDueDate;
    _status = widget.args.task?.status ?? 'todo';
  }
  ...
  Widget build(BuildContext context) {
    ...
    final editing = widget.args.task != null;
    return Scaffold(
      appBar: AppBar(title: Text(editing ? '编辑任务' : '新建任务')),
    ...
    try {
      ...
      if (widget.args.task == null) {
        ...
        await actions.updateTask(
          ...
          status: _status,
          dueDate: _dueDate,
          clearDueDate: _dueDate == null && widget.args.task!.dueDate != null,
        );
      }
      if (!mounted) return;
