...
void main() {
  ...
  test('不同 blobId 缓存相互独立', () async {
    final service = BlobAccessService(
      fetchLink: (id) async => BlobAccessLink(
        url: 'http://api/$id/signed',
        expiresAt: DateTime.now().add(const Duration(hours: 1)),
      ),
      directUrl: (id) => 'http://api/$id/direct',
    ...
    await service.resolve('b1');
    await service.resolve('b2');
    final cached = service;
    expect(await cached.resolve('b1'), 'http://api/b1/signed');
    expect(await cached.resolve('b2'), 'http://api/b2/signed');
  });
}
