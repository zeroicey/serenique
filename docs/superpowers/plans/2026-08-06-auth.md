...
export function verifySessionCookie(
  ...
  // signSessionCookie 返回完整 "<exp>.<sig>"；这里只需比对新算出的签名段
  // （直接比全串会因长度不同被 secretsEqual 判定为 tampered）。
...
- Modify: `services/api/src/env.ts`（`AUTH_TOKEN` / `SESSION_TTL` 字段——原计划在 Task 4，因 service 依赖 `env.AUTH_TOKEN` 提前到本任务；Task 4 的 env 步骤相应变为 no-op）
...
- [ ] **Step 1: env schema（已由 Task 2 完成，本步跳过）**

`env.ts` 的 `AUTH_TOKEN` / `SESSION_TTL` 字段已在 Task 2 加入：
`AUTH_TOKEN: z.string().min(32).optional()`；`SESSION_TTL: z.coerce.number().int().positive().optional()`。
**注意**：`SESSION_TTL` 是 `optional()` 而非 `.default()`——zod `.default()` 会让该字段在 `Env` 输出类型里变为必填，破坏 `createApp(env)` 的测试调用方（手写 env 对象不带 SESSION_TTL 会 typecheck 失败）。未配置时由 service 内 `?? DEFAULT_SESSION_TTL_SECONDS` 回退 30 天。本任务无 env 改动。
...
5. **两个新 DB-free 用例推迟到 Task 5 Step 1 添加**（中间件未上线前它们会红，本任务保持全绿提交）。
...
同时在 `app.test.ts` 里补上从 Task 4 推迟的两个 DB-free 用例（Task 4 已为全部 `/api` 请求带好 Bearer、`makeApp` env 已含 `AUTH_TOKEN: TEST_AUTH_TOKEN`）：
...
export async function fetchAuthStatus(): Promise<AuthStatus> {
  // throwHttpErrors:false —— 401 是「未登录」的正常信号，ky 默认会对非 2xx 抛错，
  // 会让 401 变成 unhandled rejection 而非 { authenticated:false }。仅此调用关闭。
...
```ts
  process.env.AUTH_TOKEN ??= "test-mcp-token-0123456789abcdef1";
```
...
```ts
  test("requires a bearer token on /mcp when AUTH_TOKEN is set", async () => {
    ...
    const withAuth = await app.request("/mcp", {
      ...
      headers: {
        "content-type": "application/json",
        authorization: "Bearer test-mcp-token-0123456789abcdef1",
      },
      body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "initialize", params: {} }),
    });
...
Run: `cd services/mcp && bun test`
Expected: 新用例 PASS，既有用例（不带 token 但 MCP client 用 http 连接）——注意：既有工具用例用 `StreamableHTTPClientTransport` 连接 `/mcp`，现在带 token 的测试若先于它们把 `process.env.AUTH_TOKEN` 写死，会让它们也要求 token。若既有用例失败，给 `StreamableHTTPClientTransport` 构造传入 `customHeaders: { Authorization: "Bearer test-mcp-token-0123456789abcdef1" }`（SDK 支持），保持全绿。
