...
// mock 掉 task.service 让该调用抛错，模拟无数据库环境：断言错误映射路径
// （isError: true），测试确定性成立，且不依赖真实库可用性、不产生真实写入。
...
describe("ai.tools", () => {
  ...
  test("create_task 参数 schema 接受最小输入", async () => {
    ...
    // AgentToolResult 类型上没有 isError 字段，SDK runner 只把 execute 抛出的
    // 异常标记为错误（见 pi-agent-core agent-loop.js）：无 DB（mock 抛错）时
    // run() 抛「操作失败: …」，execute reject → runner 置 isError: true。
    await expect(
