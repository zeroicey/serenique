...
describe.skipIf(!RUN_DB_TESTS)('auth + tokens integration', () => {
  ...
  test('引导期（凭证 0）：错误 SETUP_TOKEN → 403；正确 → 注册成功 + 自动登录', async () => {
    ...
    const bad = await app.request(
      ...
      json({ setupToken: 'wrong-token-0123456789abcdef' }),
    )
    ...
    const rows = await waitForAuditRows(
      and(
        eq(auditLogs.event, 'auth.register'),
        sql`${auditLogs.detail}->>'userId' = ${userRow!.id}`,
      ),
    )
  ...
  test('已有凭证后：无会话注册 → 401（即使带 SETUP_TOKEN）；带会话添加第二把凭证 → 成功', async () => {
    ...
    const startBody = await start.json()
    expect(
      startBody.data.options.excludeCredentials.some((c: { id: string }) => c.id === credentialId1),
    ).toBe(true)
  ...
  test('凭证管理：重命名设备标签（PATCH）；空串清空；不存在 → 404', async () => {
    const creds = await app.request('/api/auth/credentials', { headers: { cookie: cookie1 } })
    const items = (await creds.json()).data.items as Array<{
      id: string
    ...
    const renamed = (
      (await after.json()).data.items as Array<{ id: string; deviceLabel: string | null }>
    ...
    const rows = await waitForAuditRows(
      and(
        eq(auditLogs.event, 'auth.credential_rename'),
        sql`${auditLogs.detail}->>'id' = ${target.id}`,
      ),
    )
    expect(rows.length).toBe(1)
    trackAudit(rows[0])
    ...
    const missing = await app.request(
      '/api/auth/credentials/00000000-0000-4000-8000-000000000000',
      {
        method: 'PATCH',
        headers: { cookie: cookie1, 'content-type': 'application/json' },
  ...
  test('凭证管理：删除一把成功；删除最后一把 → 409', async () => {
    ...
    const delLast = await app.request(
      `/api/auth/credentials/${items.find((i) => i.credentialId === credentialId1)!.id}`,
      {
        method: 'DELETE',
        headers: { cookie: cookie1 },
      },
    ...
    const rows = await waitForAuditRows(
      and(
        eq(auditLogs.event, 'auth.credential_delete'),
        sql`${auditLogs.detail}->>'id' = ${second.id}`,
      ),
    )
    expect(rows.length).toBe(1)
    trackAudit(rows[0])
  ...
  test('登录失败：伪造签名 / 未知凭证 → 401 + 审计', async () => {
    ...
    const forgedRes = await app.request(
      '/api/auth/login/finish',
      json(
        { challengeId: start2Body.data.challengeId, credential: forged },
        { 'cf-connecting-ip': IP_FORGED },
      ),
    )
  ...
  test('签名 blob 链接仍公开；无签名/无凭证 → 401', async () => {
    expect((await app.request('/api/blobs/0198f6d0-9e7c-71d7-8214-2a0f7f5f2001/file')).status).toBe(
      401,
    )
