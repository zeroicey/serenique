...
export const authService = {
  ...
  async evaluateGate(input: {
    ...
    if (decision.kind === 'rejected') {
      throw new AppError(decision.code as ErrorCode, decision.message, decision.status)
    }
  ...
  async registerStart(input: RegisterStartInput & { sessionUserId: string | null }): Promise<{
    challengeId: string
  ...
  async registerFinish(
    ...
    // 找不到用户（引导脚本未跑）→ 明确报错。
    const [userRow] = await db.select().from(users).where(eq(users.id, record.userId))
    if (!userRow) {
      throw new AppError(
        ErrorCode.NOT_FOUND,
        '未找到用户，请先运行引导脚本创建用户：bun scripts/bootstrap-user.ts',
  ...
  async loginFinish(
    ...
    const [user] = await db.select().from(users).where(eq(users.id, credRow.userId))
  ...
  async deleteCredential(input: { userId: string; credentialId: string }): Promise<void> {
    const [row] = await db
      .select()
      .from(passkeyCredentials)
    ...
    await db.delete(passkeyCredentials).where(eq(passkeyCredentials.id, row.id))
    fireAuditRecord({
