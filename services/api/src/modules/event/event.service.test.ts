...
describe("assertValidEventRange — end must be strictly after start", () => {
  ...
  test("rejects end == start", async () => {
    setTestEnv();
    const { assertValidEventRange } = await import("./event.domain");
    expect(() => assertValidEventRange(START, START)).toThrow(
      /结束时间必须晚于开始时间/,
    );
  });
  ...
  test("rejects end < start", async () => {
    setTestEnv();
    const { assertValidEventRange } = await import("./event.domain");
    expect(() => assertValidEventRange(END, START)).toThrow(
      /结束时间必须晚于开始时间/,
    );
  });
});
...
describe("event zod schemas", () => {
  ...
  test("CreateEventSchema rejects an empty or too-long title", async () => {
    ...
    expect(
      CreateEventSchema.safeParse({ ...validCreate, title: "" }).success,
    ).toBe(false);
    expect(
      CreateEventSchema.safeParse({ ...validCreate, title: "   " }).success,
    ).toBe(false);
  ...
  test("CreateEventSchema rejects invalid or offset-less datetimes", async () => {
    ...
    expect(
      CreateEventSchema.safeParse({ title: "产品评审", startAt: "2026-08-05T09:00:00Z" })
        .success,
    ).toBe(false);
  });
  ...
  test("UpdateEventSchema requires at least one field", async () => {
    ...
    expect(
      UpdateEventSchema.safeParse({ title: "新标题" }).success,
    ).toBe(true);
    expect(
      UpdateEventSchema.safeParse({ isAllDay: true }).success,
    ).toBe(true);
    expect(
      UpdateEventSchema.safeParse({ endAt: "2026-08-05T12:00:00Z" }).success,
    ).toBe(true);
  });
