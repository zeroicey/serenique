...
describe('cache keys — 按请求参数序列化', () => {
  test('nearbyCacheKey 相同参数稳定、不同参数可区分', async () => {
    ...
    expect(nearbyCacheKey(base)).not.toBe(nearbyCacheKey({ ...base, lng: 116.5 }))
    expect(nearbyCacheKey(base)).not.toBe(nearbyCacheKey({ ...base, lat: 39.8 }))
    expect(nearbyCacheKey(base)).not.toBe(nearbyCacheKey({ ...base, radius: 5000 }))
    expect(nearbyCacheKey({ ...base, keyword: '咖啡' })).not.toBe(nearbyCacheKey(base))
  ...
  test('searchCacheKey 区分关键字与坐标', async () => {
    ...
    expect(searchCacheKey({ keyword: '咖啡' })).toBe(searchCacheKey({ keyword: '咖啡' }))
