...
describe("缓存 — 10 分钟 LRU", () => {
  test("相同参数二次调用命中缓存，不重复请求高德", async () => {
    ...
    expect(second.items).toEqual([
      { name: "天安门", latitude: 39.90923, longitude: 116.397428 },
    ]);
  });
...
describe("location zod schemas", () => {
  ...
  test("NearbyQuerySchema：拒绝越界坐标、非法 radius 与缺参", async () => {
    ...
    expect(
      NearbyQuerySchema.safeParse({ lng: 0, lat: 0, radius: 50001 }).success,
    ).toBe(false);
  ...
  test("NearbyQuerySchema：keyword 上限 50 字", async () => {
    const { NearbyQuerySchema } = await import("./location.types");
    expect(
      NearbyQuerySchema.safeParse({ lng: 0, lat: 0, keyword: "x".repeat(51) }).success,
    ).toBe(false);
    expect(
      NearbyQuerySchema.safeParse({ lng: 0, lat: 0, keyword: "x".repeat(50) }).success,
    ).toBe(true);
  });
  ...
  test("SearchQuerySchema：keyword 必填（1..50），lng/lat 成对出现", async () => {
    ...
    expect(
      SearchQuerySchema.safeParse({ keyword: "咖啡", lng: "116.4" }).success,
    ).toBe(false);
  });
});
