...
describe.skipIf(!RUN_DB_TESTS)('moment comment service DB integration', () => {
  ...
  test('list returns comments ascending by (created_at, id)', async () => {
    ...
    expect(listed.map((c) => c.content)).toEqual(sorted.map((c) => c.content))
  ...
  test('list embeds commentCount on moments', async () => {
    ...
    const ours = result.items.filter((m) => m.id === withComments || m.id === noComments)
  ...
  test('missing entities reject with 404', async () => {
    ...
    await expect(momentCommentService.list({ momentId: missingMoment })).rejects.toMatchObject({
      code: 'NOT_FOUND',
      status: 404,
    })
    ...
    await expect(
      momentCommentService.update({ momentId, commentId: missingComment }, { content: 'x' }),
    ).rejects.toMatchObject({ code: 'NOT_FOUND', status: 404 })
