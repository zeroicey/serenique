...
describe("moment comment schemas", () => {
  test("CreateMomentCommentSchema requires content of 1..2000 chars", async () => {
    ...
    expect(CreateMomentCommentSchema.safeParse({ content: "回看备注" }).success).toBe(
      true,
    );
    expect(CreateMomentCommentSchema.safeParse({ content: "x".repeat(2000) }).success).toBe(
      true,
    );
    expect(CreateMomentCommentSchema.safeParse({ content: "x".repeat(2001) }).success).toBe(
      false,
    );
  ...
  test("UpdateMomentCommentSchema requires content of 1..2000 chars", async () => {
    ...
    expect(UpdateMomentCommentSchema.safeParse({ content: "修改后的备注" }).success).toBe(
      true,
    );
...
describe("moment comment mappers", () => {
  ...
  test("groupCommentsByMomentId groups by momentId preserving order", async () => {
    ...
    const grouped = groupCommentsByMomentId([
      entry(m1, "c1"),
      entry(m2, "c2"),
      entry(m1, "c3"),
    ]);
