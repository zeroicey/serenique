...
describe.skipIf(!RUN_DB_TESTS)('moment service DB integration', () => {
  ...
  beforeAll(async () => {
    ...
    blobAttachmentsTable = (await import('@/modules/blob/blob.schema')).blobAttachments
  ...
  test('create attaches media blobs in order with nested blob info', async () => {
    ...
    expect(created.attachments.map((a) => a.blobId)).toEqual([imageId, audioId, videoId])
  ...
  test('list returns moments with only their own attachments', async () => {
    ...
    const ours = result.items.filter((m) => m.id === a.id || m.id === b.id)
  ...
  test('list returns moments newest-first', async () => {
    ...
    const result = await momentService.list({ page: 1, pageSize: 50 })
    const ours = result.items.filter((m) => m.id === a.id || m.id === b.id || m.id === c.id)
  ...
  test('update rejects a missing moment', async () => {
    await expect(momentService.update({ id: randomUUID(), text: '不存在' })).rejects.toThrow(
      /闪念不存在/,
    )
  })
  ...
  test('missing entities reject with 404', async () => {
    await expect(momentService.get({ id: randomUUID() })).rejects.toMatchObject({
      code: 'NOT_FOUND',
      status: 404,
    })
    ...
    await expect(momentService.delete({ id: randomUUID() })).rejects.toMatchObject({
      code: 'NOT_FOUND',
      status: 404,
    })
  ...
  test('search: Chinese keyword matches text directly', async () => {
    const a = await createSearchable(uniqueTitle('搜索-北京天气不错'), '2031-01-01T00:00:00.000Z')
    const b = await createSearchable(uniqueTitle('搜索-上海见客户'), '2031-01-02T00:00:00.000Z')
  ...
  test('search: full pinyin matches pinyin column (beijing → 北京)', async () => {
    const a = await createSearchable(uniqueTitle('搜索-北京下雨了'), '2031-01-03T00:00:00.000Z')
    const b = await createSearchable(uniqueTitle('搜索-上海有太阳'), '2031-01-04T00:00:00.000Z')
  ...
  test('search: pinyin initials match pinyin_initial column (bj → 北京)', async () => {
    const a = await createSearchable(uniqueTitle('搜索-北京出差'), '2031-01-05T00:00:00.000Z')
    const b = await createSearchable(uniqueTitle('搜索-上海开会'), '2031-01-06T00:00:00.000Z')
  ...
  test('search: English keyword matches text directly', async () => {
    ...
    const b = await createSearchable(uniqueTitle('搜索-lunch break'), '2031-01-08T00:00:00.000Z')
  ...
  test('search: mixed pinyin+English substring hits inside the compact run', async () => {
    const a = await createSearchable(uniqueTitle('搜索-北京 meeting'), '2031-01-09T00:00:00.000Z')
  ...
  test('search: total reflects the filtered set and pagination still applies', async () => {
    ...
    for (let i = 1; i <= 5; i++) {
      await createSearchable(`搜索-专用分页词-北京${i} ${RUN_TOKEN}`, `2032-01-0${i}T00:00:00.000Z`)
    }
