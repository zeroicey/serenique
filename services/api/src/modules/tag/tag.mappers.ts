...
export function toTagEntry(row: typeof tags.$inferSelect, momentCount = 0): TagEntry {
  return {
    id: row.id,
    name: row.name,
    momentCount,
...
export function toTagRelationEntry(row: typeof tagRelations.$inferSelect): TagRelationEntry {
  return {
    id: row.id,
    tagId: row.tagId,
